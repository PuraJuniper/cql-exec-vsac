...
const BASE_URL = process.env.VSAC_BASE_URL ? process.env.VSAC_BASE_URL : 'https://vsac.nlm.nih.gov';
    ...
    return getTicketGrantingTicket(username, password)
      ...
      .then((results) => {
        ...
        // There were results, so write the file first before resolving/rejecting
          return writeFile('', JSON.stringify(vsDB, null, 2), caching)
            .then(
...
function getTicketGrantingTicket(username, password) {
  ...
  return fetch(`${BASE_URL}/vsac/ws/Ticket`, options).then(res => {
    if (!res.ok) {
...
function getServiceTicket(ticketGrantingTicket) {
  ...
  return fetch(`${BASE_URL}/vsac/ws/Ticket/${ticketGrantingTicket}`, options).then(res => {
    if (!res.ok) {
      throw new Error(res.status);
    }
    return res.text();
...
function getValueSet(serviceTicket, oid, version) {
  ...
  return fetch(`${BASE_URL}/vsac/svs/RetrieveValueSet?${params}`).then(res => {
    if (!res.ok) {
      throw new Error(res.status);
    }
    return res.text();
